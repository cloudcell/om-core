...
class Engine:
    ...
    """
    SLOW_LOG_THRESHOLD = 0.01  # seconds
    ...
    def _publish_event(self, topic_suffix: str, payload: dict) -> None:
        ...
        if self._event_publisher is not None:
            try:
                self._event_publisher.publish(topic_suffix, payload, self)
            except Exception:
                import logging
    ...
    def _mark_deleted_item_in_all_rules(self, dim_name: str, deleted_item_names: set[str]) -> None:
        ...
        for cube_id, addr in changed_cell_addrs:
            self.invalidate_cell_node(cube_id, addr)
    ...
    def format_addr_label(self, cube_id: str, addr: tuple[str, ...]) -> str:
    ...
    def invalidate_cell_node(self, cube_id: str, addr: tuple[str, ...]) -> None:
        ...
        for cube in affected_cubes:
            for rid, r in self._ws.rules.items():
                ...
                if r.is_anchored and r.addr_mask is not None and self._rule_has_hierarchy_funcs(r.expression):
                    self.invalidate_cell_node(r.cube_id, r.addr_mask)
    ...
    def _record_rule_eval_profile(self, expression: str, elapsed_ms: float) -> None:
        ...
        slow = elapsed_ms > (self.SLOW_LOG_THRESHOLD * 1000.0)
        if slow:
    ...
    def recompute_dirty_nodes(self, *, include_all: bool = False, max_nodes: int | None = None, mode: str | None = None) -> int:
        ...
        *mode* controls which implementation is used:
        - ``None`` (default): ``frontier_serial_recompute`` when MT is off;
          ``frontier_auto_recompute`` when MT is on.
        ...
        try:
            ...
            if mode == "legacy_serial":
                return self.legacy_serial_recompute(max_nodes=max_nodes)
            elif mode == "parallel_forced":
                return self.frontier_parallel_forced(max_nodes=max_nodes)
                ...
                return self.frontier_serial_recompute(max_nodes=max_nodes)
    ...
    def legacy_serial_recompute(self, *, max_nodes: int | None = None) -> int:
    ...
    def frontier_serial_recompute(self, *, max_nodes: int | None = None) -> int:
    ...
    def _recompute_dirty_nodes_parallel(self, *, max_nodes: int | None = None) -> int:
        """Backward-compatible wrapper.  Delegates to ``frontier_auto_recompute``."""
        return self.frontier_auto_recompute(max_nodes=max_nodes)

    def frontier_auto_recompute(self, *, max_nodes: int | None = None) -> int:
        """Gated cube-aware parallel recompute.

        Falls back to ``frontier_serial_recompute`` when any of the
        ...
        print(f"[MT] frontier_auto_recompute: all_dirty={len(all_dirty)} cell_dirty={len(dirty_keys)} mt_enabled={self._multithread_recompute_enabled}")
        if not dirty_keys:
    ...
    def frontier_parallel_forced(self, *, max_nodes: int | None = None) -> int:
        """Benchmark/debug mode: forces parallel scheduling regardless of gate."""
        all_dirty = self._dep_graph.dirty_keys()
        dirty_keys = [key for key in all_dirty if key.startswith("cell::")]
        print(f"[MT] frontier_parallel_forced: all_dirty={len(all_dirty)} cell_dirty={len(dirty_keys)}")
        if not dirty_keys:
            return 0
        ...
        while queue and len(dependents) < max_results:
            ...
            for dep_key in self._dep_graph.dependents_of(node_key):
                ...
                if parsed is not None:
                    ...
                    try:
                        label = self.format_addr_label(dep_cube_id, dep_addr)
                    except Exception:
                        continue
        ...
        *,
        ...
        if _FLOW_TRACE_DEBUG:
            print(
                ...
                f"cube={cube.name}",
                f"addr={self.format_addr_label(cube.id, addr)}",
            ...
            *,
            ...
            if _FLOW_TRACE_DEBUG:
                print(
                    ...
                    f"expr_base={self.format_addr_label(expr_cube.id, expr_base_addr)}",
                    f"label_cube={label_cube.name}",
                    f"label_addr={self.format_addr_label(label_cube.id, label_addr)}",
                    f"expr={expr!r}",
                    f"refs={refs!r}",
                )
            for cube_name_hint, pairs in refs:
                ...
                if _FLOW_TRACE_DEBUG:
                    print(
                        ...
                        f"pairs={pairs!r}",
                        f"resolved={[(c.name, self.format_addr_label(c.id, a)) for c, a in resolved]!r}",
                    )
                for target_cube, target_addr in resolved:
                    if target_cube.id != cube.id or target_addr != addr:
                        continue
                    dep_label = self.format_addr_label(label_cube.id, label_addr)
                    target_entry = {
                        "label": dep_label,
            ...
            if expression is not None and depth < max_depth:
                ...
                for cube_name, pairs in refs:
                    ...
                    for target_cube, dep_addr in resolved:
                        ...
                        label = self.format_addr_label(target_cube.id, norm_dep_addr)
                        if label in seen_precedent_labels:
                            continue
                        precedents.append(label)
            ...
            flow.append(
                {
                    ...
                    "addr": curr_addr,
                    "addr_label": self.format_addr_label(curr_cube.id, curr_addr),
                    "depth": depth,
                    "source": source,
        ...
        *,
        ...
        return {
            "root": {
                ...
                "addr": tuple(addr),
                "addr_label": self.format_addr_label(root_cube.id, tuple(addr)),
                "value": root_value,
            },
    ...
    def add_dimension_item(self, dim_id: str, name: str, position: str = "append") -> DimensionItem:
        dim = self.require_dimension_by_id(dim_id)
    ...
    def create_dimension_item(self, dim_id: str, name: str, position: str = "append") -> DimensionItem:
        """Canonical alias for add_dimension_item (a-06 command naming)."""
        return self.add_dimension_item(dim_id, name, position=position)
    ...
    def dimension_effective_order_window(
        ...
        """
        all_ids = self.dimension_effective_order(dim_id)
        start = offset
        end = offset + limit if limit is not None else None
        return all_ids[start:end]

    def compact_dimension_graph(self, dim_id: str) -> int:
    ...
    def rename_outline_group_label(self, dim_id: str, old_label: str, new_label: str) -> None:
    ...
    def promote_row_to_col(self, view_id: str) -> None:
    ...
    def promote_topright_to_row(self, view_id: str, dim_id: str) -> None:
    ...
    def axis_items(self, dim_id: str) -> list[DimensionItem]:
    ...
    def view_row_items(self, view_id: str) -> list[DimensionItem]:
        ...
        if not view.row_dim_ids:
            return []
        return self.axis_items(view.row_dim_ids[0])
    ...
    def view_col_items(self, view_id: str) -> list[DimensionItem]:
        ...
        if not view.col_dim_ids:
            return []
        return self.axis_items(view.col_dim_ids[0])
    ...
    def _delete_rule_anchored_by_idx(self, view_id: str, row_idx: int, col_idx: int) -> bool:
        return self.delete_cell_rule(view_id, row=row_idx, col=col_idx)
    ...
    def bootstrap_dependency_graph(self) -> int:
    ...
    def recalculate_all(self, *, include_all: bool = True) -> None:
        ...
        try:
            ...
            if nodes_recomputed == 0:
                t0 = time.perf_counter()
                bootstrapped = self.bootstrap_dependency_graph()
                t1 = time.perf_counter()
                print(f"[ENGINE] Phase 3 - bootstrap_dependency_graph (cold-start): {(t1-t0)*1000:.1f} ms ({bootstrapped} cells)")
        ...
        try:
            ...
            if anchored is None:
                ...
                try:
                    ...
                    if dt > self.SLOW_LOG_THRESHOLD and _DEBUG_ENGINE:
    ...
    def _validate_cell_rule_addr(self, cube: Cube, addr: tuple[str, ...], expression: str) -> None:
        ...
        cube.set(addr, None)
        self.invalidate_cell_node(cube.id, addr)
    ...
    def delete_cell_rule(self, view_id: str, row: int, col: int) -> bool:
    ...
    def execute_command(self, command: Action) -> None:
    ...
    def mark_dirty(self) -> None:
        ...
        This is a public hook that the GUI can override to track unsaved changes.
        """
        pass
