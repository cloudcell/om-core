"""lib_runtime.repl_host — REPL client entry point.

Connects to a running runtime via transport and starts the interactive REPL.
"""
...
def _wire_repl_deps() -> dict[str, Any]:
    """Create the local command-layer dependencies needed by the REPL."""
...
def start_repl(endpoint: Any | None = None) -> None:
    """Start REPL as a pure client connecting to an existing runtime.

    Creates RemoteCommandSession over transport, then starts OpenMREPL.
    """
    ...
    deps = _wire_repl_deps()

    client = SocketTransportClient(endpoint)
    try:
        client.connect()
        session_id = client.open_session(client_type="repl")
        logger.info("REPL session opened: %s", session_id)
        ...
        # Show connection info once at startup
...
def _resolve_client_endpoint() -> Any:
    """Resolve transport endpoint for the REPL client."""
    import os
    from lib_command.core.transport_base import TransportEndpoint
